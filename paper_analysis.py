...
def bend_breath_it(age_frac, age_err, bounds, KM_metals,
                   zs, z_mids, n):
    np.random.seed()
    ...
    for i in range(len(bounds)):
        ...
        for a in range(len(age_frac[i, :])):
            ...
            if frac_rand > 0.5:
                ...
                id_rand = np.random.choice(KM_metals[ev_group].index.tolist(),
                                           int(len(KM_metals[ev_group]) * frac_rand))
                KM_slice = KM_metals.loc[id_rand].reset_index(drop=True)
                ...
                for j in range(len(zs) - 1):
                    ...
                    w_means[j], w_stds[j] = bootstrap_median(
                        np.array(KM_slice['gw'][ev_z] + 7.25), 1000)
                ...
                A[i][a] = np.nan
                B[i][a] = np.nan
    return np.nanmean(abs(A), axis=0), np.nanmean(abs(B), axis=0)
...
if __name__ == '__main__':
    ...
    # calculate the bending and breathing params
    Nboot = 1000
    As = np.zeros((len(age_frac[0, :]), Nboot))
    Bs = np.zeros((len(age_frac[0, :]), Nboot))

    with Pool(ncores) as p:
        func = partial(bend_breath_it, age_frac, age_err, bounds, KM.KM_metals, zs, z_mids)
        res = p.map(func, range(Nboot))
